...
public class ApiController implements EWrapper {
	...
	private void startMsgProcessingThread() {
		...
		class MessageProcessingThread extends Thread{
			...
			MessageProcessingThread(EClientSocket socket){ this.socket = socket; }
			@Override
			public void run(){
				while(this.socket.isConnected()){
					signal.waitForSignal();
					try {
						if (this.socket.isConnected()) reader.processMsgs();
					} catch (IOException e) {
		...
		new MessageProcessingThread(client()).start();
...
//        }).start();
    ...
    public void reqTopMktData(Contract contract, String genericTickList, boolean snapshot, boolean regulatorySnapshot, ITopMktDataHandler handler) {
    	...
    	m_client.reqMktData( reqId, contract, genericTickList, snapshot, regulatorySnapshot, Collections.<TagValue>emptyList() );
		sendEOM();
    }
    ...
    public void reqHistoricalData(Contract contract, String endDateTime, int duration, DurationUnit durationUnit, BarSize barSize, WhatToShow whatToShow, boolean rthOnly, boolean keepUpToDate, IHistoricalDataHandler handler) {
    	...
    	m_client.reqHistoricalData(reqId, contract, endDateTime, durationStr, barSize.toString(), whatToShow.toString(), rthOnly ? 1 : 0, 2, keepUpToDate, Collections.<TagValue>emptyList());
		sendEOM();
    }
	...
	public void reqNewsArticle(String providerCode, String articleId, INewsArticleHandler handler) {
		...
		m_client.reqNewsArticle(requestId, providerCode, articleId, Collections.<TagValue>emptyList());
		sendEOM();
	}
	...
	public void reqHistoricalNews( int conId, String providerCodes, String startDateTime, String endDateTime, int totalResults, IHistoricalNewsHandler handler) {
		...
		m_client.reqHistoricalNews( requestId, conId, providerCodes, startDateTime, endDateTime, totalResults, Collections.<TagValue>emptyList());
		sendEOM();
	}
    ...
    public void reqHistoricalTicks(Contract contract, String startDateTime,
        ...
        m_client.reqHistoricalTicks(reqId, contract, startDateTime, endDateTime, numberOfTicks, whatToShow, useRth, ignoreSize, Collections.<TagValue>emptyList());
    }   
