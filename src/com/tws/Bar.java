...
public final class Bar {
	...
	public Bar(int reqId, com.ib.client.Bar bar) throws ParseException
	{
		// get the time string from the bar entry
		String timestr = bar.time();

		// convert from string to double
		// time in seconds since 1970 Jan 1 00:00:00 GMT, volume in units of 100
		this.time  = com.tws.Bar.DateTimeFormat.parse((timestr.length()==8)?timestr+="  12:00:00":timestr).getTime()/1000;

		// map other bar data to fields
		this.reqId  = reqId       ;
		this.high   = bar.high()  ;
		this.low    = bar.low()   ;
		this.open   = bar.open()  ;
